...
## 为什么“设置会丢/别人看不见”以及修复方式

你之前遇到的“房主输入框里的设置一来新人就没了/别人看不到”，原因是：

- 输入框值只在本地，没有同步到后端
- 新人加入触发 `room_state` 广播时，前端会用后端默认值刷新 UI，从而覆盖你本地输入

现在修复为：

- 房主修改设置时，前端自动发送 `room_config`
- 后端保存到 room 并广播 `room_state`
- 所有人都能看到一致的设置，且不会被新人加入时覆盖
